...
class SRTParser:
    ...
    def parse_srt_file(filepath: str) -> List[SubtitleEntry]:
        ...
        for block in blocks:
            ...
            try:
                ...
                print(f"Warning: Skipping malformed SRT block: {block[:50]}... Error: {e}")
                continue
    ...
    @staticmethod
    def validate_alignment(orig_entries: List[SubtitleEntry], 
                          trans_entries: List[SubtitleEntry]) -> Tuple[bool, str]:
        """
        Check if two SRT files are properly aligned.
        Returns (is_valid, error_message)
        """
        if len(orig_entries) != len(trans_entries):
            return False, f"Entry count mismatch: Original has {len(orig_entries)} entries, Translation has {len(trans_entries)}"
        ...
        if misaligned_entries:
            if len(misaligned_entries) <= 5:
                ...
                return False, f"Timestamp misalignment in {len(misaligned_entries)} entries (first few: {', '.join(map(str, misaligned_entries[:5]))})"
